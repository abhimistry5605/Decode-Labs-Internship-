...
router.post('/', authMiddleware, async (req, res) => {
  ...
  const { name, category, description, price, stock, difficultyLevel, componentsList, projectsList, features, image, imageUrl } = req.body;
  ...
  try {
    ...
    const compList = features ? JSON.stringify(features) : (componentsList || '[]');
    const projList = features ? JSON.stringify(features) : (projectsList || '[]');
    const imgUrl = image || imageUrl || '/photos/products/electronics_equipment.jpg';
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (features) {
      ...
      updateData.componentsList = stringified;
      updateData.projectsList = stringified;
      product.componentsList = stringified;
      product.projectsList = stringified;
    } else {
